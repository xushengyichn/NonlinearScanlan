...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% clc; clear; close all;
function [Mode]=CalDamping_Polynomial_withTMD_multidegree(nTMD,mTMD,zetaTMD,omegaTMD,nodeTMD,mode_number,ifcalmode,MM_eq,KK_eq,calmodes,eig_val,eig_vec)
...
if ~exist("t_length","var")
    t_length = 100; %计算时间长度
    disp("变量t_length不存在，取默认值："+num2str(t_length))
end
...
disp(Mode)
Mode_sys = Complex_Eigenvalue_Analysis(MM,CC_aero,KK);%不考虑气动阻尼的阻尼比
disp(Mode_sys)
