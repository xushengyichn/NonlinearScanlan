...
% TMD1按照规范设计
fs = modeinfo.Freq(1);
mode_shape=zeros(length(modeinfo.eig_vec(:,1)),5);
mode_shape(:,1)=modeinfo.eig_vec(:,1);
mode_shape(:,2)=modeinfo.eig_vec(:,2);
mode_shape(:,3)=modeinfo.eig_vec(:,3);
mode_shape(:,4)=modeinfo.eig_vec(:,4);
mode_shape(:,5)=modeinfo.eig_vec(:,5);
...
[XTMD2_all, Cal_modes] = ndgrid(xTMD2_all, cal_modes);
variables = [XTMD2_all(:), Cal_modes(:)];
...
%     mode_numbers = 5;
    ...
    while and(iter <= 3, flag_convergence == 0)
        ...
        dis_temp(:,7)=u(end,:);%TMD响应
        dis_TMD=dis_temp(:,7);
        %判断计算是否收敛
        dis1 = max(maxdislocationtime(end / 8 * 6:end / 8 * 7));
        dis2 = max(maxdislocationtime(end / 8 * 7:end));
        ...
        if or(abs((dis1 - dis2) / dis2) < 0.05, dis2 < 1e-3)
            ...
            for k6=1:nModes
                seqs_allmodes=seqs_allmodes+seqs_all(:,:,k6);
            end
...
%             seqs_5modes_max=max(seqs_all(:,:,5),[],2);
            ...
            seqs_allmodes=[];
            seqs_all=[];
            dis_beam_max = std(seqs_allmodes_max_point(end / 8 * 6:end)) * sqrt(2);
            dis_TMD1_max = std(dis_TMD(end / 8 * 6:end)) * sqrt(2);
...
save nmodes_onetmd_results_loc.mat nmodes_onetmd_results_loc
