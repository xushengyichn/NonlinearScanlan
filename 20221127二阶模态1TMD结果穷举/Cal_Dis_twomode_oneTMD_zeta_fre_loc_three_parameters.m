...
zetaTMD2_all = zetaTMD1 * 0.5:0.1:zetaTMD1 * 3;
...
% parfor k1 = 1:size(variables, 1)
    ...
    while and(iter <= 3, flag_convergence == 0)
        ...
        dis_beam_mode1 = mode1 .* u(1, :);
        dis_beam_mode2 = mode2 .* u(2, :);
        dis_beam = dis_beam_mode1 + dis_beam_mode2;
        dis_beam_max = max(dis_beam, [], 2);
        [dis_beam_max_max, max_loc] = max(dis_beam_max);
        dis_beam_max_seq = dis_beam(max_loc, :);
        dis_beam_mode1=[];
        dis_beam_mode2=[];
        dis_beam=[];
        dis_beam_max=[];
        ...
        %判断计算是否收敛
        dis1 = max(dis_beam_max_seq(end / 8 * 6:end / 8 * 7));
        dis2 = max(dis_beam_max_seq(end / 8 * 7:end));

        if or(abs((dis1 - dis2) / dis2) < 0.05, dis2 < 1e-3)
            disp("计算收敛")
            flag_convergence = 1;
            dis_beam_max = std(dis_beam_max_seq(end / 8 * 6:end)) * sqrt(2);
            dis_TMD1 = std(dis_TMD(end / 8 * 6:end)) * sqrt(2);
            ...
            dis_beam_max_seq=[];
            result_temp = [dis_beam_max dis_TMD1 max_loc flag_iter];
            twomode_onetmd_dis(k1, :) = result_temp;
            %             figure
...
twomode_onetmd_results_zeta_fre_loc = [variables twomode_onetmd_dis];
