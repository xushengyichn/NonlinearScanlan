...
% 求解安装TMD后的响应
function [min_Mode_damping,result]=Compare_1_mode(numberofTMD,mTMD,zetaTMD,fTMD,xTMD,calmode)
...
MM_eq=modeinfo.MM_eq(calmode:calmode,calmode:calmode);
KK_eq=modeinfo.KK_eq(calmode:calmode,calmode:calmode);
eig_val=modeinfo.eig_val(calmode:calmode,calmode:calmode);
eig_vec=modeinfo.eig_vec(:,calmode:calmode);
...
[result]=CalDamping_Polynomial_withTMD_multidegree(nTMD,mTMD,zetaTMD,omegaTMD,xTMD,1,ifcalmode,MM_eq,KK_eq,1,eig_val,eig_vec);
Mode_damping=result.Mode.("Damping ratio");
min_Mode_damping=min(Mode_damping);
% disp(result.Mode)
