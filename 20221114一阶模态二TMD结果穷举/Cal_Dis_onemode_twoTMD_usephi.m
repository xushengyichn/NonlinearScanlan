...
%LastEditTime: 2023-01-12 16:55:48
%FilePath: \NonlinearScanlan\20221114一阶模态二TMD结果穷举\Cal_Dis_onemode_twoTMD_usephi.m
...
%     legend('1', '2', '3')
    ...
    %判断计算是否收敛
    dis1=max(dis_beam(round(end/8*6,0):round(end/8*7,0)));
    dis2=max(dis_beam(round(end/8*7,0):end));
    
        if or(abs((dis1-dis2)/dis2)<0.05,dis2<1e-3)
            disp("计算收敛")
            flag_convergence=1;
            dis_beam_max=std(dis_beam(round(end/8*6,0):end))*sqrt(2);
            dis_TMD1=std(u(2,round(end/8*6,0):end))*sqrt(2);
            dis_TMD2=std(u(3,round(end/8*6,0):end))*sqrt(2); 
            tnew=0:h:t_length;
            if iter>=5
                flag_iter=0;
            else
