...
%LastEditTime: 2022-06-03 10:20:24
%FilePath: \NonlinearScanlan\NonlinearScanlan_withTMD_singledegree.m
%Description: 本函数目的为计算单自由度经验非线性模型响应，加上单个TMD，计算其响应，现存问题为非常小的质量比就能提供很好的控制效果，暂时未发现代码编制的错误需要继续debug
%
...
%     epsilonk=237000;
    ...
    gfun = @(u, udot) Scanlan_nonlinear(u, udot, MM, CC, KK, gamma, beta, h, rho, U, D, Y1, epsilon, Y2);
    ...
    function [g, ks] = Scanlan_nonlinear(u, udot, MM, CC, KK, gamma, beta, h, rho, U, D, Y1, epsilon, Y2)
%         Caero = [-rho * U^2 * D * (Y1 * (1 - epsilon .* ([1 0] * u).^2 ./ D^2)) / U 0; 0 0];
        Caero = [-rho*U*D*Y1+rho*U/D*Y1*epsilon*u(1,1)^2 0; 0 0];
        g = (CC + Caero) * udot + (KK) * u; % Function value
        kc = CC + Caero;

%         kspring = KK + [2 * rho * U * D * Y1 * epsilon * (u(1,1)) * (udot(1,1)) / D^2 0; 0 0];
        kspring = KK + [2*rho*U/D*Y1*epsilon*u(1,1)*udot(1,1) 0; 0 0];
