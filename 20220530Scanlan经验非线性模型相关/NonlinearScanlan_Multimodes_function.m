...
%LastEditTime: 2022-05-30 19:41:05
%FilePath: \NonlinearScanlan\NonlinearScanlan_Multimodes.m
%Description: 本函数目的为计算多自由度经验非线性模型响应
%
...
%--------------------------------------------------------------------------
function  [Dis,flag,miuTMD]=NonlinearScanlan_Multimodes_function(nTMD, mTMD, cTMD, ...
    kTMD, nodeTMD, nModes,mode,T)
...
clearvars -except M C K Freq omeg mode_vec C_exp KMmapping DampingMatrixParameter zeta beta nodeondeckimport points externalforcemethod nTMD nModes MFC mTMD cTMD kTMD nodeTMD exportvideo mode T
...
clearvars -except KK MM CC matrixsize nModes nTMD mode_vec KMmapping nodeondeckimport points nodeTMD externalforcemethod MFC omeg exportvideo Freq mTMD mode T
...
phitmd=phiY(nodeTMD, KMmapping, mode_vec, mode);
% disp("TMD的质量比为"+num2str(mTMD*phitmd^2))
miuTMD=mTMD*phitmd(mode)^2;
...
for k1 =1:points
    ...
    max1= max(slice1);
    max2= max(slice2);
    if abs(max1-max2)>mean(max1+max2)*0.05
      flag =1;
    end
