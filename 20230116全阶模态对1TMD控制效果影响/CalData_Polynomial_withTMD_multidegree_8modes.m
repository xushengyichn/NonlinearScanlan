...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% clc; clear; close all;
function [modemaxdis_single, usinglemax, uallmax, output] = CalData_Polynomial_withTMD_multidegree_8modes(nTMD, mTMD, zetaTMD, omegaTMD, xTMD, mode_number, ifcalmode, MM_eq, KK_eq, calmodes, eig_val, eig_vec, t_length)
...
%     Freq = omeg / (2 * pi);
    ...
    function result = phiY(node, Mmapping, mode_vec, nModes)
        position_index1 = Mmapping.MatrixEqn(find(and(Mmapping.Node == node, Mmapping.DOF == 'UY')));

        if isempty(position_index1)
            result = zeros(1, nModes);
        else
            result = mode_vec(position_index1, 1:nModes);
        end
    ...
    function result = Peq(Pmode, mode_vec, Mmapping, P_eachpoint, points, t)
        ...
        for t01 = 1:points
            ...
            if sum(and(Mmapping.Node == t01, Mmapping.DOF == 'UY')) == 0
                ...
                position_index2 = Mmapping.MatrixEqn(find(and(Mmapping.Node == t01, Mmapping.DOF == 'UY')));
                result = result + mode_vec(position_index2, Pmode) * P_eachpoint(t01, :);
            end
