...
%Date: 2022-06-23 12:26:06
%LastEditors: Shengyi Xu xushengyichn@outlook.com
%LastEditTime: 2022-06-23 12:28:33
%FilePath: \twindeck_ID\two_order_polynomial_test.m
%Description: 由于四阶多项式结果始终不理想，尝试降为二阶多项式测试。
%
...
% xiaero=-rho*U*D*(a1+4*a2*amp/3/pi+a3*amp.^2/4)/2/m/omega0;
% figure
% plot(amp,xiaero)
...
Fre=F0;
Mass=m;
out = two_order_polynomial_NB(Fre, Mass, Zeta0, rho, D, U, a, told, P,  u0, udot0);
% close all
figure
plot(tt,xvmd,'r')
hold on
...
plot(out(:,1),out(:,2),'b')
% [psd_avg, f, psd_plot] = fft_transfer(fs,out(:,2));
% figure
% plot(f,psd_plot)
% u0 = 0.0074;
% udot0 = 0;
% [fitresult, gof] = ScanlanFit(ex, epsx);
% a = fitresult.a - Zeta0;
% b = fitresult.b;
% w0=2*pi*Fre;
% % a=rho*U*D*Y1/(-2*m*w0);b=-rho*U*D*Y1*epsilon/4/(-2*m*w0);
% Y1 = a / (rho * U * D) * (-2 * Mass * w0);
% epsilon = b * 4 / (rho * U * D * Y1) * -1 * (-2 * Mass * w0);
% Y1k=Y1;
% epsilonk=epsilon;
% % Y1k=a1;
% % epsilonk=a3*D^2/Y1k*-1;
% Y2k=0;
% ClK=0;
% out = NonlinearScanlan(Fre, Mass, Zeta0, rho, D, U, Y1k, epsilonk, Y2k, ClK, told, P, u0, udot0);
% figure
% plot(tt,xvmd,'r')
% hold on
% 
