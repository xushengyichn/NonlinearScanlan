...
m_range=linspace(12000,12000,1);
zeta_range=linspace(0.01,0.1,10);
f_range=linspace(0.8,1.1,10);
nodeondeck = readmatrix("nodeondeck.txt");
nodegap = readmatrix("nodegap.txt");
position_range=linspace(1,401,21);
position_range=nodeondeck(position_range);
...
for k1=1:size(p_range,2)
    ...
    T =T_default;
    [Dis,flag,miuTMD]=NonlinearScanlan_Multimodes_function(nTMD, mTMD, cTMD, ...
    kTMD, nodeTMD, nModes,p_range(k1),T_default);
%     while flag ==1
%         T=T+1000;
%         disp("计算未收敛，调整计算时间长度为"+num2str(T)+"秒")
%     [Dis,flag,miuTMD]=NonlinearScanlan_Multimodes_function(nTMD, mTMD, cTMD, ...
%     kTMD, nodeTMD, nModes,p_range(k1),T);
%     end
    ...
    nodeondeck = readmatrix("nodeondeck.txt");
    Max_Position=nodeondeck(Max_Position);
...
numIterations=size(TestingCases,1);    %total number of parfor iterations
...
parfor k1=1:size(TestingCases,1)
    ...
    T =T_default;
    [Dis,flag,miuTMD]=NonlinearScanlan_Multimodes_function(nTMD, mTMD, cTMD, ...
    kTMD, nodeTMD, nModes,TestingCases(k1,5),T);
    while flag ==0
        T=T+1000;
        disp("计算未收敛，调整计算时间长度为"+num2str(T)+"秒")
     [Dis,flag,miuTMD]=NonlinearScanlan_Multimodes_function(nTMD, mTMD, cTMD, ...
    ...
    [Max_Position,Max_time]=find(Dis==maxdis);
     nodeondeck = readmatrix("nodeondeck.txt");
    Max_Position=nodeondeck(Max_Position);
    location2number=find(nodeondeck==ReferenceData(TestingCases(k1,5),2))
    maxref=max(Dis(location2number,end/10*9:end));
