...
%LastEditTime: 2022-06-21 01:01:43
%FilePath: \twindeck_ID\polynomial_NB.m
%Description: 本函数目的为计算多项式模型的响应。
%
%Copyright (c) 2022 by Shengyi xushengyichn@outlook.com, All Rights Reserved. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is a function
function out = two_order_polynomial_NB(Fre, Mass, Zeta0, rho, D, U, a, t, P,  u0, udot0)
...
%     nModes = 1;
    ...
    gfun = @(u, udot) polynomial_nonlinear(u, udot, MM, CC, KK, gamma, beta, h, b);
%     gfun = @(u, udot) polynomial_nonlinear1(u, udot, MM, CC, KK, gamma, beta, rho,U,D,h, a, matrixsize, nModes)
...
%         end
%     
%         g = (CC-diagMatrix.*b(3).*u.^2-diagMatrix.*b(2).*abs(u)-b(1))*udot+(KK).*u; % Function value
%         kc= CC-diagMatrix.*b(3).*u.^2-diagMatrix.*b(2).*abs(u)-b(1);
%         kspring= KK+diagMatrix.*udot.*(-2.*b(3).*u-b(2).*u./abs(u));
%         ks = kspring + gamma.*h./(beta.*h.^2).*kc + 1./(beta.*h.^2).*MM; % Linearization
%     end
    function [g,ks]= polynomial_nonlinear(u, udot, MM, CC, KK, gamma, beta, h, b)
        g = (CC-b(3).*u.^2-b(2).*abs(u)-b(1))*udot+(KK).*u; % Function value
        kc= CC-b(3).*u.^2-b(2).*abs(u)-b(1);
        kspring= KK+udot.*(-2.*b(3).*u-b(2).*u./abs(u));
        ks = kspring + gamma.*h./(beta.*h.^2).*kc + 1./(beta.*h.^2).*MM; % Linearization
    end
%     function [g,ks]= polynomial_nonlinear2(u, udot, MM, CC, KK, gamma, beta, rho,U,D,h, a, matrixsize, nModes)
%         diagMatrix=zeros(matrixsize,matrixsize);
%         for k1=1:nModes
%             diagMatrix(k1,k1)=1;
%         end
...
%         a2=a(2);
%         a3=a(3);
%         g=KK*u-rho*U*D*(a1+a2*abs(u)/D+a3*u^2/D^2)*udot;
%         kc=-rho*U*D*(a1+a2*abs(u)/D+a3*u^2/D^2);
%         kspring=KK-rho*U*D*udot*(a2*u/D/abs(u)+2*a3*u/D^2);
%         
%         ks = kspring + gamma.*h./(beta.*h.^2).*kc + 1./(beta.*h.^2).*MM; % Linearization
%     end
%     function [g,ks]= polynomial_nonlinear1(u, udot, MM, CC, KK, gamma, beta, rho,U,D,h, a, matrixsize, nModes)
%         diagMatrix=zeros(matrixsize,matrixsize);
%         for k1=1:nModes
%             diagMatrix(k1,k1)=1;
%         end
...
%         a2=a(2);
% %         a3=a(3);
%         g=KK*u-rho*U*D*(a1+a2*abs(u)/D)*udot;
%         kc=-rho*U*D*(a1+a2*abs(u)/D);
%         kspring=KK-rho*U*D*udot*(a2*u/D/abs(u));
%         
%         ks = kspring + gamma.*h./(beta.*h.^2).*kc + 1./(beta.*h.^2).*MM; % Linearization
%     end
end
