...
%LastEditTime: 2022-10-06 11:20:09
%FilePath: \NonlinearScanlan\20221007双幅桥多项式模型参数识别(增加振幅上下限限制点)\twindeck_ID.m
...
fname = ['SZTD-110-case2-22.3-fasan-3501']; %分析工况对应文件名
...
if ~isempty(my_table.up_epsx0(isexist))
    alertstr=(fname+"上游零振幅下的阻尼比为："+num2str(my_table.up_epsx0(isexist))+",是否需要修改？(0/不修改，1/修改)");
    decidevalue=input(alertstr);
    if decidevalue==1
        up_epsx0=input("设置上游梁0振幅时阻尼比为：");
        up_ex=[0;up_ex];
        up_epsx=[up_epsx0;up_epsx];
    else
        up_epsx0=my_table.up_epsx0(isexist);
        up_ex=[0;up_ex];
        up_epsx=[up_epsx0;up_epsx];
    end
else
    disp(fname+"未设置该工况零振幅下的阻尼比，请输入");
    up_epsx0=input("设置上游梁0振幅时阻尼比为：");
    up_ex=[0;up_ex];
    up_epsx=[up_epsx0;up_epsx];
end

if ~isempty(my_table.up_epsx_max(isexist))
    alertstr=(fname+"上游较大振幅下的阻尼比为："+num2str(my_table.up_epsx_max(isexist))+",是否需要修改？(0/不修改，1/修改)");
    decidevalue=input(alertstr);
    if decidevalue==1
        up_ex_end=up_ex(end);
        up_epsx_end=up_epsx(end);
        disp("目前上游识别到最大振幅为："+up_ex_end);
        disp("目前上游识别到最大振幅对应阻尼比为："+up_epsx_end);
        up_ex_max=input("设置上游梁大振幅：");
        up_epsx_max=input("设置上游梁大振幅时对应阻尼比为：");
        up_ex=[up_ex;up_ex_max];
        up_epsx=[up_epsx;up_epsx_max];
    else
        up_ex_max=my_table.up_ex_max(isexist);
        up_epsx_max=my_table.up_epsx_max(isexist);
        up_ex=[up_ex;up_ex_max];
        up_epsx=[up_epsx;up_epsx_max];
    end
else
    disp(fname+"未设置该工况大振幅下的阻尼比，请输入");
    up_ex_end=up_ex(end);
    up_epsx_end=up_epsx(end);
    disp("目前上游识别到最大振幅为："+up_ex_end);
    disp("目前上游识别到最大振幅对应阻尼比为："+up_epsx_end);
...
% decidevalue=input("是否需要限制上游大振幅阻尼比:(0/不需要，1/需要)");
% if decidevalue==1
%     up_ex_end=up_ex(end);
%     up_epsx_end=up_epsx(end);
%     disp("目前上游识别到最大振幅为："+up_ex_end);
...
figure; plot(up_ex, up_epsx, 'g'); hold on; plot(up_ex, up_epsxeq, 'r'); title('瞬时阻尼结果 计算结果(绿)+多项式拟合结果(红)')
...
up_amp = 0.0001:0.0001:up_ex(end)*2; up_amp = up_amp';
...
if ~isempty(my_table.down_epsx0(isexist))
    alertstr=(fname+"下游零振幅下的阻尼比为："+num2str(my_table.down_epsx0(isexist))+",是否需要修改？(0/不修改，1/修改)");
    decidevalue=input(alertstr);
    if decidevalue==1
        ...
        down_epsx0=my_table.down_epsx0(isexist);
                down_ex=[0;down_ex];
        down_epsx=[down_epsx0;down_epsx];
    end
else
...
if ~isempty(my_table.down_epsx_max(isexist))
    alertstr=(fname+"下游较大振幅下的阻尼比为："+num2str(my_table.down_epsx_max(isexist))+",是否需要修改？(0/不修改，1/修改)");
    decidevalue=input(alertstr);
    if decidevalue==1
        down_ex_end=down_ex(end);
        down_epsx_end=down_epsx(end);
        disp("目前下游识别到最大振幅为："+down_ex_end);
        disp("目前下游识别到最大振幅对应阻尼比为："+down_epsx_end);
        down_ex_max=input("设置下游梁大振幅：");
        down_epsx_max=input("设置下游梁大振幅时对应阻尼比为：");
        down_ex=[down_ex;down_ex_max];
        down_epsx=[down_epsx;down_epsx_max];
    else
        down_ex_max=my_table.down_ex_max(isexist);
        down_epsx_max=my_table.down_epsx_max(isexist);
                down_ex=[down_ex;down_ex_max];
        down_epsx=[down_epsx;down_epsx_max];
    end
else
...
down_amp = 0.0001:0.0001:down_ex(end)*2; down_amp = down_amp'; down_epsxeq = polyval(down_bepsx, down_amp);
figure
...
down_amp =  0.0001:0.0001:down_ex(end)*2; down_amp = down_amp';
...
if recorddata == 1
    ...
    T.up_epsx0 = up_epsx0;
    T.up_ex_max = up_ex_max;
    T.up_epsx_max = up_epsx_max;
    T.down_start = down_index_start;
    T.down_end = down_index_end;
    T.down_epsx0 = down_epsx0;
    T.down_ex_max = down_ex_max;
    T.down_epsx_max = down_epsx_max;
    T.up_m = m_up;
